...
def load_cnn_params(file):
    ...
    assert isinstance(params['num_channels'], int) and \
        (params['num_channels'] >= 4) and \
        is_power_of_two(params['num_channels'])
    ...
    assert isinstance(params['batch_size'], int) and \
        (0 < params['batch_size'] < 5000) and \
        is_power_of_two(params['batch_size'])
...
class NavDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # read data
        self.scan_file_names, self.scan_data = self.read_data(
            data_path, 'scans_local', file_name)
        self.sub_goal_file_names, self.sub_goal_data = self.read_data(
            data_path, 'sub_goals_local', file_name)
        self.final_goal_file_names, self.final_goal_data = self.read_data(
            data_path, 'final_goals_local', file_name)
        self.vel_file_names, self.vel_data = self.read_data(
            data_path, 'velocities', file_name)
    ...
    def __getitem__(self, idx):
        if self.preload:
            ...
            scan = self.normalize_scan(
                np.load(self.scan_file_names[idx]),
                self.normalization_method
            )
            sub_goal = self.normalize_sub_goal(
                np.load(self.sub_goal_file_names[idx]),
                self.normalization_method
            )
            final_goal = self.normalize_final_goal(
                np.load(self.final_goal_file_names[idx]),
                self.normalization_method
            )
...
class CNNModel(nn.Module):
    ...
    def __init__(self, in_channels: int, num_hiddens: int, num_scan_points: int = 360):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=num_hiddens//4,
        ...
        ################## full connect net model: ###################  # noqa: E266
    ...
    def forward(self, scan, goal):
        ...
        ###### Start of full connect net #######  # noqa: E266
