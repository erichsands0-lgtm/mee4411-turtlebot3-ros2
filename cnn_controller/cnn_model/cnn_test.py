...
from cnn_model import load_cnn_params, NavDataset, CNNModel
import numpy as np
...
def main(argv):
    ...
    if (len(argv) != NUM_ARGS):
        print("usage: python cnn_test.py [PARAM_FILE] [MDL_PATH] [DEV_PATH]")
        exit(-1)
    ...
    # load the weights
    checkpoint = torch.load(mdl_path, map_location=device, weights_only=True)
