...
# import the model
from cnn_model import set_seed, load_cnn_params, NavDataset, CNNModel
...
def main(argv):
    ...
    if(len(argv) != NUM_ARGS):
        print("usage: python cnn_train.py [PARAM_FILE] [MDL_PATH] [TRAIN_PATH] [DEV_PATH]")
        exit(-1)
    ...
    # Load parameters
    dnn_params = load_cnn_params(param_file)
    ...
    # instantiate a model
    model = CNNModel(in_channels=1, num_hiddens=dnn_params['num_channels'])
    ...
    for epoch in (pBar := tqdm(range(start_epoch+1, num_epochs))):
        ...
        pBar.set_postfix_str(
            f'Training loss: {train_epoch_loss:.4f}, Validation loss: {valid_epoch_loss:.4f}'
        )
