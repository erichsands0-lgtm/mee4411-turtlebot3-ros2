...
from wheel_odometry.tb3_kinematics import TB3Kinematics


# test calculate_wheel_change
def test_calculate_wheel_change():
    ...
    for d_true, d, name in zip(data_true, data, names):
        assert np.abs(d - d_true) < 1e-6, \
            f"Data {name} has the wrong value ({d:.6f} instead of {d_true:.6f})"


# test calculate_displacement
def test_calculate_displacement():
    kn = TB3Kinematics('waffle')
    ...
    for ds, dt, ds_true, dt_true in zip(delta_s, delta_theta, delta_s_true, delta_theta_true):
        assert np.abs(ds - ds_true) < 1e-6, \
            f"Linear displacement is wrong ({ds:.6f} instead of {ds_true:.6f})"
        assert np.abs(dt - dt_true) < 1e-6, \
            f"Angular displacement is wrong ({dt:.6f} instead of {dt_true:.6f})"


# test calculate_pose
def test_calculate_pose():
    kn = TB3Kinematics('burger')
    ...
    for p, p_true, n in zip(pose, pose_true, names):
        assert np.abs(p - p_true) < 1e-6, \
            f"Pose {n} is wrong ({p:.6f} instead of {p_true:.6f})"
